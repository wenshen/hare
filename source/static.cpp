...
Static::Static(int _start, int _numNodes, double distances[100][100], int vel[100][100], int _theGoalNode, double _lambda, double given[4], int gameLength) {
	source = _start;
	destination = -1;
	progress = 0.0;//0.0
    ...
    r = 0.5;//(float)(rand() % 2) * 0.9;
    g = 0.5;//(float)(rand() % 2) * 0.9;
    b = 0.5;//(float)(rand() % 2) * 0.9;    
}

Static::~Static() {
    printf("simple worth = %lf\n", worth);
}

void Static::copyAgent(Static *_a) {
	source = _a->source;
...
void Static::choose(Node *_world[100]) {
    ...
    if (currentDestination >= 0) {
		...
		for (j = 0; j < _world[source]->numLinks; j++) {
			...
			tmp = (cost[source][_world[source]->links[j]]*GAS_COST) + (_world[source]->toll[j] / 100.0) + toGo[_world[source]->links[j]];
            pthread_mutex_unlock( &count_mutex );
			if ((tmp < best) && _world[source]->linkStatus[j]) {
				best = tmp;
				l = j;
			}
		}
...
int Static::update(double _progress, double t) {
	progress = _progress;
    worth -= t * GAS_COST;
	int rval = 0;
	if (progress >= 1.0) {
...
void Static::runStatic(Node *_world[100], int dest) {
    pthread_mutex_lock( &count_mutex );
	toGo[dest] = 0;
...
void Static::pickDestination(Node *_world[100]) {
    ...
    runStatic(_world, currentDestination);
